...
| Инфра    | Монорепо (`backend/`, `frontend/`). Деплой: Docker (бэкенд на порту 3000), фронт — сборка локально + rsync. |
...
│   ├── render-start.js # запуск для production (БД + bot.js)
...
│   ├── .env            # переменные окружения (создать вручную)
│   └── package.json
├── docker-compose.yml  # деплой бэкенда (сервис app, порт 3000)
├── deploy.sh           # локальный скрипт деплоя
├── deploy-server.sh    # на сервере: git pull + docker compose
├── README.md
...
| `PORT`                | нет         | Порт сервера. Локально и на проде используется **3000** (в production задаётся в `backend/.env` на сервере).           |
...
Сервер поднимется на `http://localhost:3000` (порт задаётся в `backend/.env`, по умолчанию в config — 3002; для единообразия лучше указать 3000).
...
- **Деплой на сервер:** см. [DEPLOY.md](./DEPLOY.md). Бэкенд разворачивается через Docker (один порт **3000**), фронт собирается локально и копируется по rsync.
