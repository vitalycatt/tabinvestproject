  ...
  actions: {
    ...
    async fetchUsers() {
      ...
      try {
        ...
        this.users = data.users || response.users || [];

        // Обновляем статистику из response.data.stats (API возвращает data: { users, pagination, stats })
        const stats = data.stats || response.stats;
        if (stats) {
          this.stats.totalUsers = stats.total ?? 0;
          this.stats.activeUsers = stats.activeToday ?? 0;
          this.stats.newUsers = stats.newThisWeek ?? 0;
        }
